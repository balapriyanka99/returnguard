...
class Auditor:
    ...
    def fraud_label_safety(self):
        ...
        # Scan only deterministic data-access/MCP layers for actual runtime references.
        # Agent prompts/contracts may legitimately mention "fraud_labels" only to block it.
        scan_roots = [
            self.repo_root / "returnguard" / "intelligence",
            self.repo_root / "returnguard" / "mcp",
        ]
        ...
        self.add(
            "13B",
            "ERROR",
            "Deterministic runtime data-access/MCP code references evaluation-only fraud_labels",
            suspicious,
            note=(
                "Agent prompts/contracts are intentionally excluded because they may name fraud_labels "
                "only in deny-lists or safety instructions. Intelligence and MCP must have zero references."
            ),
        )
