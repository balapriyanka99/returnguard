...
class Auditor:
    ...
    def inspection_product(self):
        ...
        self.add("07D", "ERROR", "Serial-required present item lacks expected serial", self.query(f"""
        SELECT r.return_id, r.product_id, p.serial_required, i.expected_serial, i.returned_serial
        FROM {self.fq("return_requests")} r
        JOIN {self.fq("return_inspections")} i USING(return_id)
        JOIN {self.fq("product_attributes")} p USING(product_id)
        WHERE p.serial_required = TRUE
          AND i.item_present = TRUE
          AND i.expected_serial IS NULL
        """))
    ...
    def economics(self):
        self.add("08A", "WARNING", "Case economics disagree with product baseline", self.query(f"""
        SELECT
        ...
        """), note="Warning only because case-level overrides may be intentional.")
        ...
        if root.exists():
            for p in root.rglob("*.py"):
                ...
                for n, line in enumerate(txt.splitlines(), 1):
                    if "fraud_labels" in line:
                        suspicious.append({
                            "file": str(p.relative_to(self.repo_root)),
                            "line": n,
                            "text": line.strip()[:300],
                        })
        self.add("13B", "ERROR", "Runtime Python code references evaluation-only fraud_labels", suspicious,
                 note="Expected zero references under returnguard/.")
